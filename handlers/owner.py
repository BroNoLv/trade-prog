...
def register_owner_handlers(dp: Dispatcher):
    # Статистика
    dp.register_message_handler(show_owner_stats_menu, lambda m: m.text == "📈 Общая статистика")
    dp.register_message_handler(show_stats_by_period, lambda m: m.text == "📊 Общая статистика", state="*")
    
    # Управление токенами
    dp.register_message_handler(manage_tokens_start, lambda m: m.text == "🔑 Управление токенами")
    dp.register_message_handler(create_token_start, lambda m: m.text == "🔐 Создать токен", state="*")
    dp.register_message_handler(process_token_role, state=OwnerStates.waiting_token_role)
    dp.register_message_handler(list_tokens, lambda m: m.text == "📋 Список токенов")
    dp.register_message_handler(deactivate_token_start, lambda m: m.text == "❌ Деактивировать токен", state="*")
    dp.register_message_handler(process_deactivate_token, state=OwnerStates.deactivating_token)
    
    # Управление курсом
    dp.register_message_handler(manage_exchange_rate, lambda m: m.text == "💱 Курс USDT")
    dp.register_message_handler(set_rate_manual_start, lambda m: m.text == "✏️ Установить курс", state="*")
    dp.register_message_handler(process_manual_rate, state=OwnerStates.setting_exchange_rate)
    dp.register_message_handler(auto_update_rate, lambda m: m.text == "🔄 Автообновление курса")
    
    # Управление пользователями
    dp.register_message_handler(manage_users, lambda m: m.text == "👥 Управление пользователями")
    
    # Удаление сделок
    dp.register_message_handler(delete_specific_deal_start, lambda m: m.text == "🗑️ Удалить сделку", state="*")
    dp.register_message_handler(delete_specific_deal, state=OwnerStates.deleting_deal)
    
    # Кнопка "Назад"
    dp.register_message_handler(handle_back_from_tokens, lambda m: m.text == "🔙 Назад", state="*")
