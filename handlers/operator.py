...
def register_operator_handlers(dp: Dispatcher):
    dp.register_message_handler(create_deal_start, lambda m: m.text == "➕ Создать заявку")
    dp.register_message_handler(process_deal_amount, state=OperatorStates.waiting_deal_amount)
    dp.register_message_handler(show_operator_stats, lambda m: m.text == "📊 Статистика")
    dp.register_message_handler(search_transactions_start, lambda m: m.text == "🔍 Поиск транзакций")
    dp.register_message_handler(show_active_deals_operator, lambda m: m.text == "📋 Активные заявки")
